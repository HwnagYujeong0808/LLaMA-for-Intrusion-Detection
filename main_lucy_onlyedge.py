...
class LLMGraphTransformer(nn.Module):
    def __init__(self, model_name="TinyLlama/TinyLlama-1.1B-intermediate-step-1431k-3T", device="cpu"):
        ...
        self.classifier = nn.Linear(64, 9).to(self.device)

    def forward(self, edge_features):
        edge_emb = self.edge_fc(edge_features)  # (Batch, 77) -> (Batch, 64)
        edge_emb = F.relu(edge_emb)
        edge_emb = self.edge_dropout(edge_emb)
        
        # Final classification layer
        output = self.classifier(edge_emb)  # (Batch, 64) -> (Batch, 9)
        return output
    ...
    #     return batch_text # Tokenize input text
...
def fit(args):
    ...
    for epoch in range(200):
        ...
        for batch_idx in range(num_batches):
            ...
            # print(f"edge_batch: {edge_batch}")
            # print(f"batch_labels: { batch_labels}")

            logits = llm_graph_transformer(edge_batch)
    ...
    # Test Loop
    test_loss, test_f1, test_label_acc = evaluate(llm_graph_transformer, edge_list, edge_feat, label, test, labels, loss_fn, device)
    print_log(f"Test Loss: {test_loss:.4f}, Test F1 Score: {test_f1:.4f},  Label Accuracy: {test_label_acc}")
